...
async def test_extraction_with_real_images():
    ...
    for image_path in test_images:
        try:
            ...
            print(f"📝 OCR Teksta garums: {len(ocr_result.text)} simboli")
            
            # Testē regex ekstraktēšanu
            extracted_data = await extraction_service.extract_data(ocr_result.text)
            ...
            print(f"🏦 Bankas konts: {extracted_data.bank_account}")
            ...
            print(f"\n📊 Confidence Score: {extracted_data.confidence_score:.1f}")
